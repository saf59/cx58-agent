...
mod en {
    ...
    async fn project_knowledge() {
        let state = shared_state().await;
        let events = send_and_collect(&state, &build_request("How does the monitoring system track construction progress?", "en")).await;
        assert_no_error(&events);
        assert_completed(&events);
        assert_text_chunks(&events);
    }
...
mod de {
    ...
    async fn project_knowledge() {
        let state = shared_state().await;
        let events = send_and_collect(&state, &build_request("Wie verfolgt das Überwachungssystem den Baufortschritt?", "de")).await;
        assert_no_error(&events);
        assert_completed(&events);
        assert_text_chunks(&events);
    }
